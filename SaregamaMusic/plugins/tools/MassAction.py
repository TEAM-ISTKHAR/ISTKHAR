...
async def banall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_ISTKHARBOT = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'banall' can only be executed by the group owner {owner_ISTKHARBOT}.")
        return
...
async def unbanall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_ISTKHARBOT = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'unbanall' can only be executed by the group owner {owner_ISTKHARBOT}.")
        return
    confirm_msg = await message.reply(
...
async def unmuteall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_ISTKHARBOT = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'unmuteall' can only be executed by the group owner {owner_ISTKHARBOT}.")
        return
...
async def muteall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_ISTKHARBOT = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'muteall' can only be executed by the group owner {owner_ISTKHARBOT}.")
        return
    confirm_msg = await message.reply(
...
async def kickall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_ISTKHARBOT = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'kickall' can only be executed by the group owner {owner_ISTKHARBOT}.")
        return
    confirm_msg = await message.reply(
...
async def unpinall(client: Client, message: Message):
    ...
    async for admin in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
        if admin.status == enums.ChatMemberStatus.OWNER:
            ...
            owner_ISTKHARBOT = admin.user.mention
    if user_id != owner_id and user_id not in SUDOERS:
        await message.reply_text(f"Hey {message.from_user.mention}, 'unpinall' can only be executed by the group owner {owner_ISTKHARBOT}.")
        return
    confirm_msg = await message.reply(

...
from ISTKHARX.utils import first_page, second_page
from ISTKHARX.utils.database import get_lang
from ISTKHARX.utils.decorators.language import LanguageStart, languageCB
from ISTKHARX.utils.inline.help import help_back_markup, private_help_panel
...
THUNDER_Text = ("ᴄʜᴏᴏsᴇ ᴛʜᴇ ᴄᴀᴛᴇɢᴏʀʏ ғᴏʀ ᴡʜɪᴄʜ ʏᴏᴜ ᴡᴀɴɴᴀ ɢᴇᴛ ʜᴇʟᴩ.\nᴀsᴋ ʏᴏᴜʀ ᴅᴏᴜʙᴛs ᴀᴛ <a href={0}>sᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ</a>\n\nᴀʟʟ ᴄᴏᴍᴍᴀɴᴅs ᴄᴀɴ ʙᴇ ᴜsᴇᴅ ᴡɪᴛʜ: <code>/</code>")

@app.on_callback_query(filters.regex("THUNDERXaditi") & ~BANNED_USERS)
@languageCB
async def first_pagexx(client, CallbackQuery, _):
    menu_next = second_page(_)
    try:
        await CallbackQuery.message.edit_text(THUNDER_Text, reply_markup=menu_next)
        return
    except:
        return
